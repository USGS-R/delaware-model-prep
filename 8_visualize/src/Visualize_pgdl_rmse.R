...
plot_label <- c('+ space awareness', '+ pretraining', '+ time awareness','plain neural network')
  ggplot(data = subset_mod_dat, aes(x = dat_availability,
     ...
     #scale_x_continuous(label = c("2%", "10%", "100%")) +
...
ggplot(data = non_summer_mod_dat, aes(x = rmse, colour = model, group = season)) +
   geom_col(stat = "count", size = 1)
class(non_summer_mod_dat$model)
