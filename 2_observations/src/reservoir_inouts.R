...
get_inout_obs <- function(
  ...
  flows <- flow_dat %>%
    ...
    pivot_longer(-datetime, names_to='site_id', values_to='flow_cfs') %>%
    select(site_id, date = datetime, flow_cfs) %>%
...
get_inout_obs_cannonsville <- function() {
  ...
  ggplot(inouts, aes(x=date, y=flow_cfs, color=location)) + geom_line() + facet_grid(site_id ~ .)
  ggplot(inouts, aes(x=date, y=temp_degC, color=location)) + geom_point(size=0.2) + facet_grid(site_id ~ .)
}
...
get_inout_sntemp <- function(res_id, inflow_segs, outflow_segs) {
  ...
  preds %>%
    mutate(direction = case_when(
      ...
      TRUE ~ 'outflow')) %>%
    write_csv(file=sprintf('9_collaborator_data/res/inouts_%s.csv', res_id))
}
get_inout_sntemp( # cannonsville
  res_id = 'nhdhr_120022743',
  inflow_segs = c('1559','1557'),
  # res_segs = c('1561','1560','1562'),
  outflow_segs = '1566')
get_inout_sntemp( # pepacton
  res_id = 'nhdhr_151957878',
