...
munge_npy_dat <- function(in_file, model_name, seg_vector) {
  ...
  names(dat_in) <- c('2005', '2006', '2007', '2008', '2009', '2010', '2011', '2012',
...
combine_preds_obs <- function(obs_ind, sntemp_ind, ann_npy, rnn_npy, rgnc_npy, rgnc_ptnc_npy, out_file){
  ...
  ANN <- munge_npy_dat(in_file = ann_npy, model = 'ANN') # %>%
    #mutate(seg_id_nat = as.character(seg_id_nat), date = as.Date(date))

  # 2) + time
  RNN <- munge_npy_dat(in_file = rnn_npy, model = 'RNN') #%>%
   # mutate(seg_id_nat = as.character(seg_id_nat), date = as.Date(date))

  # 3) + space
  RGNC <- munge_npy_dat(in_file = rgnc_npy, model = 'RGNC') #%>%
   # mutate(seg_id_nat = as.character(seg_id_nat), date = as.Date(date))

  # 4) + pre_training
  RGNC_ptrn <- munge_npy_dat(in_file = rgnc_ptnc_npy, model = 'RGCN_ptrn')# %>%
   # mutate(seg_id_nat = as.character(seg_id_nat), date = as.Date(date))
